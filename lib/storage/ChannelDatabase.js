...
class ChannelDatabase {
    ...
    join(user, channelName, key) {
        ...
        if (channel.isInviteOnly && !channel.onInviteList(user)) {
            user.send(this.server.host, protocol.errors.inviteOnly, user.nick, channel.name, ':Cannot join channel (+i)');
            return;
        }

        if (channel.isBanned(user)) {
            user.send(this.server.host, protocol.errors.banned, user.nick, channel.name, ':Cannot join channel (+b)');
            return;
        }

        if (channel.isLimited && channel.users.length >= channel.userLimit) {
            user.send(this.server.host, protocol.errors.channelIsFull, user.nick, channel.name, ':Channel is full.');
            return;
        }
        ...
        if (channel.topic) {
            user.send(this.server.host, protocol.reply.topic, user.nick, channel.name, ':' + channel.topic);
        } else {
            user.send(this.server.host, protocol.reply.noTopic, user.nick, channel.name, ':No topic is set');
        }

        user.send(this.server.host, protocol.reply.nameReply, user.nick, channel.type, channel.name, ':' + channel.names);
        user.send(this.server.host, protocol.reply.endNames, user.nick, channel.name, ':End of /NAMES list.');
    }
