...
module.exports = {
    PRIVMSG(user, target, message) {
        ...
        if (!target || target.length === 0) {
            user.send(user.server.host, protocol.errors.noRecipient, ':No recipient given');
        } else if (!message || message.length === 0) {
            user.send(user.server.host, protocol.errors.noTextToSend, ':No text to send');
        } else if (user.server.channelTarget(target)) {
            const channel = user.server.channels.find(target);
            if (!channel) {
                user.send(user.server.host, protocol.errors.noSuchNick, user.nick, target, ':No such nick/channel');
            } else if (channel.isModerated && !user.isVoiced(channel)) {
                user.send(user.server.host, protocol.errors.cannotSend, channel.name, ':Cannot send to channel');
