...
module.exports = {
    WHO(user, target) {
        ...
        if (user.server.channelTarget(target)) {
            ...
            if (!channel) {
                user.send(user.server.host, protocol.errors.noSuchChannel, user.nick, ':No such channel');
            } else {
                ...
                user.send(user.server.host, protocol.reply.endWho, user.nick, channel.name, ':End of /WHO list.');
            }
        } else {
            ...
            user.send(user.server.host, protocol.reply.endWho, user.nick, target, ':End of /WHO list.');
        }
    }
}
