        ...
        if (user.server.channelTarget(target)) {
            ...
            if (!channel) {
                ...
                if (modes[0] === '+') {
                    ...
                    user.send(user.server.host, irc.reply.endBan, user.nick, channel.name, ':End of Channel Ban List');
                }
            } else {
                user.send(user.server.host, irc.reply.channelModes, user.nick, channel.name, channel.modes);
            }
        } else {
