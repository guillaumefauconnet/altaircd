...
module.exports = {
    INVITE(user, nick, channelName) {
        ...
        if (!targetUser) {
            user.send(user.server.host, protocol.errors.noSuchNick, user.nick, nick, ':No such nick/channel');
            return;
        } else if (channel) {
            if (channel.isInviteOnly && !user.isOp(channel)) {
                user.send(user.server.host, protocol.errors.channelOpsReq, user.nick, channel.name, ":You're not channel operator");
                return;
            } else if (channel.onInviteList(targetUser)) {
                user.send(user.server.host, protocol.errors.userOnChannel, user.nick, targetUser.nick, ':User is already on that channel');
                return;
            }
        ...
        user.send(user.server.host, protocol.reply.inviting, user.nick, targetUser.nick, channelName);
